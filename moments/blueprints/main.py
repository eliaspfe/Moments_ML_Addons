...
def show_notifications():
    ...
    pagination = db.paginate(
        stmt.order_by(Notification.created_at.desc()), page=page, per_page=per_page
    )
...
def upload():
    if request.method == "POST":
        ...
        filename_s = resize_image(
            f, filename, current_app.config["MOMENTS_PHOTO_SIZES"]["small"]
        )
...
def collect(photo_id):
    ...
    if current_user != photo.author and photo.author.receive_collect_notification:
        push_collect_notification(
            user=current_user, photo_id=photo_id, receiver=photo.author
        )
    return redirect(url_for(".show_photo", photo_id=photo_id))
...
def delete_photo(photo_id):
    ...
    next_photo = db.session.scalar(
        base_stmt.filter(Photo.created_at < photo.created_at).order_by(
...
def translate_comment(comment_id):
    ...
    try:
        response = requests.post(
            API_URL, headers=headers, json={"inputs": comment.body}
        )
        data = response.json()
        translation = (
